...
#if (Number_of_ISRs > 0) && defined isr_ButtonLeft__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_ButtonLeft__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_ButtonLeft;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_ButtonLeft__INTC_NUMBER, (ISR_MIN_PRI - 1));
    CyIntSetPriority(isr_ButtonLeft__INTC_NUMBER, ISR_SCALE(1));
    //\PF
	NVIC_INT_ENABLE(isr_ButtonLeft__INTC_NUMBER);
#elif Number_of_ISRs > 0
#error  The Interrupt isr_ButtonLeft could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_ButtonLeft */

#if (Number_of_ISRs > 1) && defined isr_ButtonRight__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_ButtonRight__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_ButtonRight;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_ButtonRight__INTC_NUMBER, (ISR_MIN_PRI - 2));
    CyIntSetPriority(isr_ButtonRight__INTC_NUMBER, ISR_SCALE(2));
    //\PF
	NVIC_INT_ENABLE(isr_ButtonRight__INTC_NUMBER);
#elif Number_of_ISRs > 1
#error  The Interrupt isr_ButtonRight could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_ButtonRight */
